...
Auth legacy: `PLAYER`. Phạm vi: self-service — không cần `club_id` để **duyệt** giải và kết quả.

### Quyền auth mặc định

`tournament.view`, `statistics.view`, `player.view`, `player.update` (chỉ bản thân), `skill_level.view_private`, `skill_level.request_change`, `team.view`, `team.lineup.submit`, `team.standings.view`.

**Không có:** `tournament.create/update/delete`, `club.*`, `booking.*`, `finance.*`, `court.*`, `user.manage`.

**Scope SELF:** `tournament.view` và `statistics.view` cho VĐV **không yêu cầu** `profiles.club_id` — cho phép account-only duyệt giải trước khi gia nhập CLB (`rbac.matchesSelfScope`).

**SQL đồng bộ Production:** `docs/v5/PHASE_37_PLAYER_DEFAULT_RBAC.sql`

### Menu sidebar

Nhóm: **CLB & Huấn luyện** (CLB của tôi, Danh sách huấn luyện viên, Đăng ký gói học), **Giải đấu** (Tổng quan, Danh sách giải, Vận động viên / Đội, Kết quả), Hỗ trợ. Hồ sơ: footer sidebar (không trùng nhóm profile).

Home mặc định: `/my-club` (chưa CLB) hoặc `/tournament` (đã gắn CLB).
