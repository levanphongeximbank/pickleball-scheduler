-- Court Operations canonical booking lifecycle 01. READ-ONLY PRECHECK.
-- LOCAL AUTHORING ONLY. DO NOT APPLY TO STAGING OR PRODUCTION.
-- BOOKING_LIFECYCLE_MIGRATION_VERSION=20260816160000
-- Requires Phase 3B reservation authority + Batch1/Batch2 canonical read packages.
-- Does not edit Phase 3A / Phase 3B / D4 / Batch1 / Batch2 SQL.
...
BEGIN
  ...
  -- Owner-type vocabulary must already accept the booking owner.
  IF public.court_resource_map_gateway_owner_type('booking') IS DISTINCT FROM 'booking' THEN
    RAISE EXCEPTION
      'PRECHECK_FAIL court_resource_map_gateway_owner_type does not map booking owner';
  END IF;

  -- Refuse re-apply drift: this package must not already exist.
  IF to_regclass('public.court_operations_bookings') IS NOT NULL THEN
    v_conflict := array_append(v_conflict, 'public.court_operations_bookings');
  END IF;
  IF to_regclass('public.court_operations_booking_commands') IS NOT NULL THEN
    v_conflict := array_append(v_conflict, 'public.court_operations_booking_commands');
  END IF;
  IF EXISTS (
    SELECT 1
    ...
    WHERE n.nspname = 'public'
      AND p.proname LIKE 'court_operations_booking%'
  ) THEN
    v_conflict := array_append(v_conflict, 'public.court_operations_booking*() functions');
  END IF;

  IF cardinality(v_conflict) > 0 THEN
    RAISE EXCEPTION
      'PRECHECK_FAIL booking lifecycle objects already present — refuse re-apply drift: %',
      array_to_string(v_conflict, ', ');
  END IF;

  RAISE NOTICE 'PRECHECK_OK court_resource_canonical_booking_lifecycle_01';
END
$$;
