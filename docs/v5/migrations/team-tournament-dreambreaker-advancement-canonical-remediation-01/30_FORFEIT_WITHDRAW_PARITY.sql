...
-- â”€â”€â”€ 1. Schema extensions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€â”€ 2. Technical score config (single source in tournament settings) â”€
...
-- â”€â”€â”€ 3. Confirmed result guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- â”€â”€â”€ 6. Forfeit ops helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
begin
  if p_sub_match.id is null then
    return jsonb_build_object(
      ...
      'blockCode', 'NOT_FOUND',
      'blockMessage', 'KhÃ´ng tÃ¬m tháº¥y tráº­n con.'
    );
  end if;
  ...
  if public.team_tournament_sub_match_is_confirmed_normal(p_sub_match) then
    return jsonb_build_object(
      ...
      'blockMessage', 'Tráº­n con Ä‘Ã£ xÃ¡c nháº­n â€” khÃ´ng thá»ƒ apply forfeit trá»±c tiáº¿p.',
  ...
  if not v_can_apply then
    return jsonb_build_object(
      ...
      'blockMessage', 'KhÃ´ng cÃ³ quyá»n xá»­ thua ká»¹ thuáº­t.',
      'subMatchVersion', p_sub_match.version
    );
  end if;
  ...
  if p_matchup.status not in ('published', 'in_progress', 'completed', 'locked') then
    return jsonb_build_object(
      ...
      'blockMessage', 'Matchup chÆ°a sáºµn sÃ ng cho forfeit.',
      'subMatchVersion', p_sub_match.version
    );
  end if;
...
-- â”€â”€â”€ 7. Apply forfeit (TT-4 enhanced) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
begin
  ...
  if coalesce(trim(p_forfeit_reason), '') = '' then
    return json_build_object('ok', false, 'code', 'forfeit_reason_required', 'error', 'LÃ½ do báº¯t buá»™c.');
  end if;
  ...
  if v_matchup.id is null then
    return json_build_object('ok', false, 'code', 'NOT_FOUND', 'error', 'Matchup khÃ´ng tá»“n táº¡i.');
  end if;

  if p_scope <> 'sub_match' or p_sub_match_id is null then
    return json_build_object('ok', false, 'code', 'VALIDATION', 'error', 'TT-4 pilot: chá»‰ há»— trá»£ scope sub_match qua RPC nÃ y.');
  end if;
  ...
  if v_sub_match.id is null then
    return json_build_object('ok', false, 'code', 'NOT_FOUND', 'error', 'Sub-match khÃ´ng tá»“n táº¡i.');
  end if;
  ...
  if public.team_tournament_sub_match_is_confirmed_normal(v_sub_match) then
    return json_build_object(
      ...
      'error', 'Tráº­n con Ä‘Ã£ xÃ¡c nháº­n â€” khÃ´ng thá»ƒ apply forfeit trá»±c tiáº¿p.'
    );
  end if;

  if p_forfeiting_team_id not in (v_matchup.team_a_id, v_matchup.team_b_id) then
    return json_build_object('ok', false, 'code', 'VALIDATION', 'error', 'Äá»™i forfeit khÃ´ng há»£p lá»‡.');
  end if;
...
-- â”€â”€â”€ 8. Team withdrawal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
begin
  ...
  if coalesce(trim(p_reason), '') = '' then
    return json_build_object('ok', false, 'code', 'withdrawal_reason_required', 'error', 'LÃ½ do rÃºt giáº£i báº¯t buá»™c.');
  end if;
  ...
  if v_team.id is null then
    return json_build_object('ok', false, 'code', 'NOT_FOUND', 'error', 'Äá»™i khÃ´ng tá»“n táº¡i.');
  end if;
  if v_team.withdrawn then
    return json_build_object('ok', false, 'code', 'ALREADY_WITHDRAWN', 'error', 'Äá»™i Ä‘Ã£ rÃºt giáº£i.');
  end if;
...
-- â”€â”€â”€ 9. Grants â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
comment on function public.team_tournament_withdraw_team is 'TT-4 team withdrawal â€” forfeits unplayed sub-matches, preserves confirmed results';
