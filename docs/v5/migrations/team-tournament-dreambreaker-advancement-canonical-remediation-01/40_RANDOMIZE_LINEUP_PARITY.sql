-- Production parity for team_tournament_randomize_lineup (active UI requires it).
-- Source of truth: docs/v5/PHASE_TT2D_RANDOMIZE_LOCK_WORKFLOW.sql
-- Classification: REQUIRED_BY_ACTIVE_UI (not optional for Setup randomize button).
-- Idempotent on Staging. DO NOT APPLY without Owner GO.
-- Phase TT-2D â€” Missing-lineup policy, server-side randomize, lock workflow
-- Prerequisite: PHASE_23C + TT-1B + TT-2B + TT-2C on staging
-- Safe to re-run (create or replace). Staging only.

-- â”€â”€â”€ Policy normalization â”€â”€â”€
...
-- â”€â”€â”€ Greedy random lineup builder (uses TT-2C validator) â”€â”€â”€
...
begin
  ...
  if v_team.id is null then
    return jsonb_build_object('ok', false, 'code', 'NOT_FOUND', 'message', 'KhÃ´ng tÃ¬m tháº¥y Ä‘á»™i.');
  end if;
  ...
  if v_matchup.id is null then
    return jsonb_build_object('ok', false, 'code', 'NOT_FOUND', 'message', 'KhÃ´ng tÃ¬m tháº¥y matchup.');
  end if;
  ...
  loop
    ...
    if v_gender_req = 'mixed_pair' and v_discipline.player_count = 2 then
      ...
      if not (v_selections ? v_discipline.external_discipline_id) then
        return jsonb_build_object(
          ...
          'message', format('%s: khÃ´ng Ä‘á»§ cáº·p nam/ná»¯ Ä‘á»ƒ random.', v_discipline.name)
        );
      end if;
      continue;
    end if;
    ...
    if v_count <> v_discipline.player_count then
      return jsonb_build_object(
        'ok', false,
        'code', 'randomize_failed',
        'message', format('%s: khÃ´ng Ä‘á»§ VÄV há»£p lá»‡ Ä‘á»ƒ random.', v_discipline.name)
      );
    end if;
  ...
  if not coalesce((v_validation->>'ok')::boolean, false) then
    return jsonb_build_object(
      ...
      'message', coalesce(v_validation->>'message', v_validation->>'error', 'KhÃ´ng táº¡o Ä‘Æ°á»£c Ä‘á»™i hÃ¬nh random.'),
...
-- â”€â”€â”€ Matchup lineup ops snapshot (BTC UI) â”€â”€â”€
...
-- â”€â”€â”€ Mark forfeit/manual policy (no full forfeit in TT-2D) â”€â”€â”€
...
-- â”€â”€â”€ RPC: randomize lineup (server-side only) â”€â”€â”€
...
begin
  ...
  if v_policy <> 'random' then
    return json_build_object('ok', false, 'code', 'POLICY_BLOCKED', 'message', 'ChÃ­nh sÃ¡ch khÃ´ng cho random.');
  end if;
  ...
  if v_matchup.status not in ('scheduled', 'lineup_open') then
    return json_build_object('ok', false, 'code', 'MATCHUP_LOCKED', 'message', 'Matchup Ä‘Ã£ khÃ³a.');
  end if;

  if not public.team_tournament_deadline_passed(v_matchup.lineup_lock_at, v_now) then
    return json_build_object('ok', false, 'code', 'DEADLINE_NOT_PASSED', 'message', 'ChÆ°a háº¿t háº¡n ná»™p Ä‘á»™i hÃ¬nh.');
  end if;
  ...
  if v_team.id is null or p_team_id not in (v_matchup.team_a_id, v_matchup.team_b_id) then
    return json_build_object('ok', false, 'code', 'NOT_FOUND', 'message', 'Äá»™i khÃ´ng thuá»™c matchup.');
  end if;
  ...
  if public.team_tournament_lineup_is_submitted(v_lineup.status) then
    return json_build_object('ok', false, 'code', 'ALREADY_SUBMITTED', 'message', 'Äá»™i Ä‘Ã£ ná»™p Ä‘á»™i hÃ¬nh.');
  end if;

  if v_lineup.status = 'locked' then
    return json_build_object('ok', false, 'code', 'LINEUP_LOCKED', 'message', 'Äá»™i hÃ¬nh Ä‘Ã£ khÃ³a.');
  end if;
  ...
  if not coalesce((v_built->>'ok')::boolean, false) then
    ...
    perform public.team_tournament_write_audit(
      ...
      jsonb_build_object(
        ...
        'message', coalesce(v_built->>'message', 'Random tháº¥t báº¡i'),
    ...
    return json_build_object(
      'ok', false,
      'code', coalesce(v_built->>'code', 'randomize_failed'),
      'message', coalesce(v_built->>'message', 'KhÃ´ng táº¡o Ä‘Æ°á»£c Ä‘á»™i hÃ¬nh random.'),
      'manualPending', true,
...
-- â”€â”€â”€ RPC: lock matchup (policy-aware, TT-2D) â”€â”€â”€
...
begin
  ...
  if not coalesce((v_ops->>'canLock')::boolean, false) then
    return json_build_object(
      'ok', false,
      'code', 'CANNOT_LOCK',
      'message', 'ChÆ°a Ä‘á»§ Ä‘iá»u kiá»‡n khÃ³a Ä‘á»™i hÃ¬nh.',
      'lineupOps', v_ops
    );
  end if;
  ...
  loop
    ...
    if v_policy = 'random' then
      ...
      return json_build_object(
        'ok', false,
        'code', 'MANUAL_PENDING',
        'message', 'Äá»™i thiáº¿u lineup â€” cáº§n xá»­ lÃ½ thá»§ cÃ´ng trÆ°á»›c khi khÃ³a.',
        'teamId', v_team_id
      );
    end if;
  end loop;
...
-- â”€â”€â”€ get_setup: attach lineup ops per matchup for BTC â”€â”€â”€
...
begin
  ...
  v_header := public.team_tournament_resolve_header(p_tournament_id);
  if v_header.id is null then
    return json_build_object('ok', false, 'code', 'NOT_FOUND', 'error', 'KhÃ´ng tÃ¬m tháº¥y giáº£i.');
  end if;
