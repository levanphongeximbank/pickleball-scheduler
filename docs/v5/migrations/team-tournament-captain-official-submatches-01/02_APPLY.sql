...
--
-- Extends team_tournament_get_captain_portal to embed matchups[].subMatches
-- from public.team_tournament_sub_matches for viewer-scoped matchups only,
-- gated by publication status (published / in_progress / completed).
-- Core field shape matches get_setup (without manage-only *Ops fields).
-- ═══════════════════════════════════════════════════════════════════
...
comment on function public.team_tournament_get_captain_portal(text, integer) is
  'Scoped captain portal reader. Includes published matchups[].subMatches (get_setup core field parity).';
