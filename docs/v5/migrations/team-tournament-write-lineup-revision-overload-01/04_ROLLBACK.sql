...
--
-- Restores the stale TT1B 12-arg overload (reintroduces ambiguity if
-- callers again use untyped 12-arg calls). Does NOT mutate lineup rows.
-- Does NOT restore pre-CAS save/submit bodies — those remain CAS-safe;
-- only the write_lineup_revision call sites are reverted to 12-arg shape
-- matching team-tournament-lineup-revision-cas-01 (pre-overload fix).
-- ═══════════════════════════════════════════════════════════════════

-- A. Recreate stale 12-arg TT1B writer (no actor_role column write)
...
-- B. Revert save/submit helper calls to pre-fix 12-arg shape (CAS body preserved).
--    WARNING: this reintroduces the live "is not unique" failure on Staging
--    while both overloads coexist.
