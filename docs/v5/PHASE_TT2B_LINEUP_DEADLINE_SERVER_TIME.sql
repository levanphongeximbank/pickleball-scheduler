-- Phase TT-2B — Server-side lineup deadline fields for get_setup
-- Prerequisite: PHASE_23C + PHASE_TT1B applied on staging
-- Safe to re-run (create or replace)

-- ─── Helper: deadline permission snapshot (PostgreSQL now() is SoT) ───
create or replace function public.team_tournament_lineup_deadline_fields(
  p_lineup_lock_at timestamptz,
  p_matchup_status text,
  p_lineup_status text,
  p_lineup_locked_at timestamptz default null
)
returns jsonb
language plpgsql
volatile
...
declare
  ...
  v_lineup_status text := coalesce(nullif(trim(p_lineup_status), ''), 'not_submitted');
  v_matchup_status text := coalesce(nullif(trim(p_matchup_status), ''), 'scheduled');
  v_editable boolean;
begin
  v_editable := v_lineup_status in ('not_submitted', 'draft', 'submitted');

  if p_lineup_locked_at is not null
    or v_lineup_status in ('locked', 'published', 'withdrawn', 'overridden', 'expired')
    or v_matchup_status in ('locked', 'published', 'in_progress', 'completed')
  then
    v_status := 'locked';
  elsif not v_editable then
    v_status := 'locked';
  elsif p_lineup_lock_at is null then
    v_status := 'before';
    v_can_save := true;
    v_can_submit := true;
  elsif now() < p_lineup_lock_at then
    v_status := 'before';
    v_can_save := true;
    v_can_submit := true;
  elsif now() >= p_lineup_lock_at
    and now() < p_lineup_lock_at + interval '1 second'
  then
    -- Exact deadline boundary: mutations blocked (now() >= lock_at in save RPC)
    v_status := 'at';
    v_can_save := false;
    v_can_submit := false;
  else
  ...
  return jsonb_build_object(
    'lineupDeadline', p_lineup_lock_at,
    'canSaveDraft', v_can_save,
    'canSubmit', v_can_submit,
    'deadlineStatus', v_status
  );
end;
$$;

-- ─── RPC: team_tournament_get_setup (TT-2B deadline extensions) ───
...
grant execute on function public.team_tournament_lineup_deadline_fields(
