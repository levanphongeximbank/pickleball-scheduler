...
--
-- Q0A_ONLY_PREPARED_RESTORE=SUPPORTED
--   PREPARED + no RPC privilege snapshot rows for this batch:
--   Q1A never committed → RPC caller EXECUTE was never revoked/changed.
--   Restore exact Q0A service_role Club table DML snapshot only.
--   Do NOT invent/replay RPC GRANT statements.
--   Terminal state: ABORTED.
--
-- Q1A_COMMITTED_PRE_APPLY_RESTORE=SUPPORTED
--   PREPARED/QUIESCED/DRAINED + RPC privilege snapshot EXECUTE rows present:
--   Replay EXACT captured EXECUTE grants including service_role mutation
--   privileges if Q1 changed them, then restore exact service_role Club table
--   DML from wave5_cutover_table_privilege_snapshot.
--
-- QUIESCED/DRAINED without RPC EXECUTE snapshot rows: FAIL_CLOSED
--   (those states require Q1A to have committed).
-- PREPARED with non-EXECUTE RPC snapshot rows but zero EXECUTE: FAIL_CLOSED
--   (ambiguous snapshot).
--
-- GRANT only snapshotted privileges; respect is_grantable.
-- No generic full-DML GRANT. No generic authenticated/public/service_role set.
-- RESTORE_REQUIRES_EXPLICIT_BATCH_ID=YES
-- LATEST_SNAPSHOT_IMPLICIT_RESTORE=DENIED
-- RESTORE_FINAL_ACL_EQUALS_SNAPSHOT=YES (when RPC snapshot path applies)
...
DECLARE
  ...
  v_state text;
  v_kind text;
  v_rpc_any_rows int := 0;
  v_rpc_execute_rows int := 0;
  v_q0a_only boolean := false;
  v_updated int := 0;
  v_tbl text;
  v_priv text;
BEGIN
  ...
  -- Detect whether Q1A committed an RPC privilege snapshot for this batch.
  IF to_regclass('public.wave5_cutover_rpc_privilege_snapshot') IS NULL THEN
    v_rpc_any_rows := 0;
    v_rpc_execute_rows := 0;
  ELSE
    SELECT count(*) INTO v_rpc_any_rows
    FROM public.wave5_cutover_rpc_privilege_snapshot s
    WHERE s.batch_id = v_batch;

    SELECT count(*) INTO v_rpc_execute_rows
    FROM public.wave5_cutover_rpc_privilege_snapshot s
    WHERE s.batch_id = v_batch
      AND s.privilege_type = 'EXECUTE';
  END IF;

  IF v_rpc_any_rows > 0 AND v_rpc_execute_rows < 1 THEN
    RAISE EXCEPTION 'WAVE5_RESTORE_ABORT: ambiguous RPC snapshot for batch % any_rows=% execute_rows=0 — FAIL_CLOSED',
      v_batch, v_rpc_any_rows;
  END IF;

  IF v_state IN ('QUIESCED', 'DRAINED') THEN
    IF v_rpc_execute_rows < 1 THEN
      RAISE EXCEPTION 'WAVE5_RESTORE_ABORT: state=% requires Q1A RPC privilege snapshot EXECUTE rows for batch %',
        v_state, v_batch;
    END IF;
    v_q0a_only := false;
  ELSIF v_state = 'PREPARED' AND v_rpc_execute_rows < 1 THEN
    -- Q0A committed; Q1A never committed. RPC caller EXECUTE unchanged.
    v_q0a_only := true;
  ELSE
    v_q0a_only := false;
  END IF;
  ...
  IF v_q0a_only THEN
    IF to_regclass('public.wave5_cutover_table_privilege_snapshot') IS NULL THEN
      RAISE EXCEPTION 'WAVE5_RESTORE_ABORT: Q0A_ONLY_PREPARED requires wave5_cutover_table_privilege_snapshot for batch %',
        v_batch;
    END IF;
    RAISE NOTICE 'WAVE5_RESTORE_Q0A_ONLY_PREPARED batch=% RPC_SNAPSHOT_ABSENT=YES restore_table_dml_only=YES RPC_GRANT_REPLAY=DENIED',
      v_batch;
  ELSE
    IF EXISTS (
      SELECT 1
  ...
  RAISE NOTICE 'WAVE5_RESTORE_LEGACY_WRITES_OK batch=% q0a_only=% replayed_execute_grants=% replayed_table_dml_grants=% state=ABORTED POST_APPLY_LEGACY_ACL_RESTORE=DENIED RESTORE_FINAL_TABLE_DML_EQUALS_SNAPSHOT=YES',
    v_batch, v_q0a_only, v_granted, v_tbl_granted;
END $$;
