-- PICK_VN Notification Phase 1.6
-- Worker operations + observability + environment isolation + queue hygiene
-- Apply on Staging only. Do NOT apply to Production.
-- Rollback: docs/supabase-notification-phase16-rollback.sql
--
-- Extends Phase 1.5. Live Email/SMS/Zalo/Web Push remain disabled.
-- Production worker execution remains blocked in Phase 1.6.

-- ─── Runtime config keys (staging) ──────────────────────────────────
...
VALUES
  ...
  ('allow_replay', 'true', now()),
  ('allow_cancel', 'true', now()),
  ('allow_stale_lease_recovery', 'true', now()),
...
BEGIN
  ...
  v_env := coalesce(public.notification_runtime_config_get('environment'), 'staging');
