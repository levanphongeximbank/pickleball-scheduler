...
| --- | --- | --- |
| Identity & Access | BOUND | wraps `createIdentityEvidenceFromIdentityAdapter`; `resolveSubjectIdentity` is Identity-domain point lookup by canonical subjectId only. Tenant is not venue: `tenantId` is never copied from `venueId`, and missing status is never synthesized as ACTIVE. |
