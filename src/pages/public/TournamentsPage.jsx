...
import { loadPublicTournamentsPageResult } from "../../features/public-portal/services/publicTournamentsRankingsDataSource.js";
...
export default function TournamentsPage() {
  ...
  const [tournamentsResult, setTournamentsResult] = useState(null);
  ...
  useEffect(() => {
    ...
    loadPublicTournamentsPageResult()
      .then((next) => {
        if (!cancelled) setTournamentsResult(next);
      })
      .finally(() => {
  ...
  return (
    <Box sx={{ ...publicSectionSx, pt: { xs: 4, md: 6 } }}>
      <Container maxWidth="lg">
        ...
        {loading || !tournamentsResult ? (
          <PublicLoadingState title="Đang tải danh sách giải đấu…" />
        ) : (
          <>
            <PublicDataSourceNotice
            ...
            (!Array.isArray(tournamentsResult.data) || tournamentsResult.data.length === 0) ? (
              ...
              <PublicUnavailableState
                ...
                onAction={retry}
              />
            ) : tournamentsResult.status === PUBLIC_DATA_RESULT_STATUS.EMPTY ||
              (Array.isArray(tournamentsResult.data) && tournamentsResult.data.length === 0) ? (
              <PublicEmptyState
                title="Chưa có giải đấu công khai"
                message="Danh sách giải đấu đang trống. Vui lòng quay lại sau khi có dữ liệu mới."
                actionLabel="Thử lại"
                onAction={retry}
              />
            ) : (
                    ...
                    {TOURNAMENT_TYPE_FILTERS.map((f) => (
                      <Chip
                        ...
                        sx={{
                          ...
                          color:
                            typeFilter === f.id ? PUBLIC_COLORS.primary : PUBLIC_COLORS.textMuted,
                          cursor: "pointer",
                          "&:focus-visible": {
                            outline: `2px solid ${PUBLIC_COLORS.lime}`,
                            outlineOffset: 2,
