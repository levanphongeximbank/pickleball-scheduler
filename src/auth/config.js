...
export function isRbacEnabledFromEnv() {
  const fromMeta = import.meta.env?.VITE_RBAC_ENABLED;
  const fromProcess =
    typeof globalThis.process !== "undefined" ? globalThis.process.env?.VITE_RBAC_ENABLED : undefined;
  const raw = fromMeta !== undefined && String(fromMeta).trim() !== "" ? fromMeta : fromProcess;
