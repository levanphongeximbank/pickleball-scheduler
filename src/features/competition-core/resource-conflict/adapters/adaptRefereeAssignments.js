/**
 * CORE-14 Phase 1F — dormant referee assignment adapter (shape-only).
 * Produces REFEREE occupancies only. No roster search or referee selection.
 */
 ...
 */
export function adaptRefereeAssignmentsToResourceOccupancies(input) {
  ...
  for (let i = 0; i < records.length; i += 1) {
    ...
    if (record == null || typeof record !== "object" || Array.isArray(record)) {
      ...
      diagnostics.push(
        createInputDiagnostic({
          code: INPUT_DIAGNOSTIC_CODE.ADAPTER_RECORD_INVALID,
          message: "referee assignment record must be an object",
          path,
    ...
    const keyInput = rec.resourceKey ?? {
      ...
      resourceId: rec.refereeId ?? rec.resourceId,
      scopeType: rec.scopeType,
      scopeId: rec.scopeId,
    };
    const keyResult = requireResourceKeyOfKind(keyInput, RESOURCE_KIND.REFEREE, path);
    if (!keyResult.ok) {
      rejectedRecordCount += 1;
    ...
    const sourceRecordIdentity =
      ...
      optionalIdentity(rec.refereeAssignmentId) ||
    ...
    const occupancyId = createAdapterOccupancyId({
      ...
      occupancyRole: RESOURCE_KIND.REFEREE,
    });
    ...
    occupancies.push(
      createResourceOccupancy({
        ...
        source: OCCUPANCY_SOURCE.REFEREE_ASSIGNMENT,
        metadata: {
          adapter: "referee",
          sourceRecordIdentity,
        },
  ...
  return createAdapterResult({
    ...
    sourceContractVersion,
    metadata: {
      adapter: "adaptRefereeAssignmentsToResourceOccupancies",
      partialAdaptation,
      rosterLookupPerformed: false,
    },
  });
}
