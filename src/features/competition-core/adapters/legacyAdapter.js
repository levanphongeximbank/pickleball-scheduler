...
export async function executeCompetitionEngine(input, options = {}) {
  ...
  if (plan.executionPath === "v2") {
    ...
    if (input.engineType === COMPETITION_ENGINE_TYPE.STANDINGS) {
      const payload = plan.input.payload && typeof plan.input.payload === "object"
        ? plan.input.payload
        : {};
      const bridge = evaluateCanonicalStandingsRuntime({
        ...
        legacyExecutor: () => {
          const legacyExecutor = options.legacyExecutor;
          if (typeof legacyExecutor !== "function") {
            return { ok: false, errors: ["Legacy executor not configured"], standing: [] };
          }
          return legacyExecutor(plan.input);
        },
        executionMode: "shadow",
      ...
      return wrapLegacyEngineResult({
        ...
        legacyResult: bridge.legacyResult,
        executionPath: "v2",
        warnings: bridge.outputPreserved ? bridge.warnings : ["Standings adapter output parity check failed"],
      });
    }
