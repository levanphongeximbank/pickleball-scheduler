/**
 * Shared helpers for CM-04 contracts (pure, deterministic).
...
export function failContract(code, message, details) {
  throw new CompetitionConfigurationError(code, message, details);
}
...
export function stableContentFingerprint(value) {
  ...
  return `${COMPETITION_CONFIGURATION_FINGERPRINT_ALGORITHM.prefix}${(hash >>> 0)
    .toString(16)
    .padStart(8, "0")}`;
}
