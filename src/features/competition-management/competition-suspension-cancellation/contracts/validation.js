/**
 * Field-level validation errors + validation result envelope (CM-07).
 */
...
export function buildExplanation(
  errors,
  validSummary = "Competition lifecycle operation is valid."
) {
  ...
  const summary =
    sorted.length === 0
      ? validSummary
      : sorted.length === 1
        ? `Competition lifecycle invalid: ${reasons[0]}`
        : `Competition lifecycle invalid (${sorted.length} field errors).`;
