/**
 * Capability-local in-memory CompetitionArchive repository (CM-08).
 *
...
import { isCompetitionArchiveRecord } from "../contracts/archive.js";
import {
  archiveScopeKey,
  createArchiveIdempotencyStorageKey,
} from "../contracts/identity.js";
import { COMPETITION_ARCHIVE_INITIAL_REVISION } from "../constants/revision.js";
import { COMPETITION_ARCHIVE_REPOSITORY_PORT_METHODS } from "../ports/repositoryPort.js";

export { archiveScopeKey };

/**
 * @returns {object}
 */
export function createInMemoryCompetitionArchiveRepository() {
  /** @type {Map<string, object>} recordId -> frozen clone */
  const byId = new Map();
  /** @type {Map<string, string>} scopeKey -> current recordId */
  const byScopeCurrent = new Map();
  /** @type {Map<string, string[]>} scopeKey -> ordered recordIds */
   ...
   */
  function appendArchiveActionAtomically(params = {}) {
    const record = params && typeof params === "object" ? params.record : null;

    if (!isCompetitionArchiveRecord(record)) {
      return validationFail([
        createFieldError(
          "record",
          COMPETITION_ARCHIVE_ERROR_CODE.MALFORMED_ARCHIVE_RECORD,
          "cannot append malformed CompetitionArchiveRecord",
          {}
        ),
      ]);
    }
    ...
    const scope = archiveScopeKey(stored.tenantId, stored.competitionId);

    if (byId.has(stored.recordId)) {
      return validationFail([
        createFieldError(
          "recordId",
          COMPETITION_ARCHIVE_ERROR_CODE.DUPLICATE_ARCHIVE_RECORD,
          "duplicate archive record identity in repository",
    ...
    if (currentId == null) {
      if (stored.revision !== COMPETITION_ARCHIVE_INITIAL_REVISION) {
        return validationFail([
          createFieldError(
            "revision",
            COMPETITION_ARCHIVE_ERROR_CODE.REPOSITORY_CONFLICT,
            `first archive record requires revision=${COMPETITION_ARCHIVE_INITIAL_REVISION}`,
      ...
      if (stored.previousRecordId !== null) {
        return validationFail([
          createFieldError(
            "previousRecordId",
            COMPETITION_ARCHIVE_ERROR_CODE.REPOSITORY_CONFLICT,
            "first archive record requires previousRecordId=null",
            {}
          ),
        ]);
      }
      ...
      if (!prior) {
        return validationFail([
          createFieldError(
            "previousRecordId",
            COMPETITION_ARCHIVE_ERROR_CODE.REPOSITORY_CONFLICT,
            "current archive pointer is broken",
            { currentId }
          ),
        ]);
      }
      ...
      if (
        ...
        return validationFail([
          createFieldError(
            "tenantId",
            COMPETITION_ARCHIVE_ERROR_CODE.CROSS_TENANT_ACCESS,
            "archive lineage tenant/competition mismatch",
            {}
          ),
        ]);
      }
    ...
    if (stored.idempotencyKey) {
      const idemKey = createArchiveIdempotencyStorageKey(
        stored.tenantId,
      ...
      if (owner && owner !== stored.recordId) {
        return validationFail([
          createFieldError(
            ...
            "idempotency key already bound to another archive record",
   ...
   */
  function findArchiveRecord(params = {}) {
    ...
    if (
      ...
      return validationFail([
        createFieldError(
          "recordId",
          COMPETITION_ARCHIVE_ERROR_CODE.CROSS_TENANT_ACCESS,
          "archive record not accessible for this tenant/competition",
          {}
        ),
      ]);
    }
   ...
   */
  function findCurrentArchiveState(params = {}) {
    if (!isNonEmptyString(params.tenantId) || !isNonEmptyString(params.competitionId)) {
      return validationFail([
        createFieldError(
          "tenantId",
    ...
    const scope = archiveScopeKey(params.tenantId, params.competitionId);
   ...
   * @param {{ tenantId: string, competitionId: string }} params
   */
  function listArchiveHistory(params = {}) {
    if (!isNonEmptyString(params.tenantId) || !isNonEmptyString(params.competitionId)) {
      return validationFail([
        createFieldError(
          "tenantId",
  ...
  function findByIdempotencyKey(params = {}) {
    ...
    const key = createArchiveIdempotencyStorageKey(
      params.tenantId,
  ...
  return Object.freeze({
    ...
    __portMethods: COMPETITION_ARCHIVE_REPOSITORY_PORT_METHODS,
  });
}
