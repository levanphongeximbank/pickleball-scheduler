/**
 * Shared helpers for CM-08 contracts (pure, deterministic).
...
import { COMPETITION_ARCHIVE_FINGERPRINT_ALGORITHM } from "../constants/comparison.js";
...
export function failContract(code, message, details) {
  throw new CompetitionArchiveError(code, message, details);
}
...
export function stableContentFingerprint(value) {
  ...
  return `${COMPETITION_ARCHIVE_FINGERPRINT_ALGORITHM.prefix}${(hash >>> 0)
    .toString(16)
    .padStart(8, "0")}`;
}
...
/**
 * Detect forbidden secret / binary / dump markers in manifest payloads.
 * @param {unknown} value
 * @param {string[]} [path]
 * @returns {string[]}
 */
export function collectForbiddenManifestMarkers(value, path = []) {
  /** @type {string[]} */
  const hits = [];
  ...
  if (typeof value === "string") {
    ...
    if (
      lower.includes("data:application/") ||
      lower.includes("-----begin ") ||
      /eyJ[a-z0-9_-]+\.[a-z0-9_-]+\.[a-z0-9_-]+/i.test(value)
    ) {
      hits.push(path.join(".") || "(root)");
    }
    return hits;
  }
  ...
  if (Array.isArray(value)) {
    value.forEach((item, idx) => {
      hits.push(...collectForbiddenManifestMarkers(item, [...path, String(idx)]));
    });
    return hits;
  }
  for (const [key, child] of Object.entries(value)) {
    ...
    hits.push(...collectForbiddenManifestMarkers(child, [...path, key]));
  }
  return hits;
}
