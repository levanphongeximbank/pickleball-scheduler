...
const SURFACE_DESCRIPTORS = deepFreeze({
  ...
  [PUBLIC_PORTAL_SURFACE_ID.PUBLIC_HOME]: createPublicPortalSurfaceDescriptor({
    ...
    notes:
      "Presentation-only tournament cards; no standings/scoring. Auth CTA → /login. Not safe for EC-01 UI remediation (mixed mock + high-collision shell).",
  }),
  ...
  [PUBLIC_PORTAL_SURFACE_ID.PUBLIC_TOURNAMENTS]: createPublicPortalSurfaceDescriptor({
    ...
    shellPathHint: SHELL,
    evidenceReferences: ["src/pages/public/TournamentsPage.jsx", SERVICE, MOCK],
    notes: "Does not recalculate standings. Deep links stay on list (/tournaments).",
  }),
  ...
  [PUBLIC_PORTAL_SURFACE_ID.PUBLIC_CLUBS]: createPublicPortalSurfaceDescriptor({
    ...
    shellPathHint: SHELL,
    evidenceReferences: ["src/pages/public/ClubsPage.jsx", SERVICE, MOCK],
  }),
  ...
  [PUBLIC_PORTAL_SURFACE_ID.PUBLIC_COURTS]: createPublicPortalSurfaceDescriptor({
    ...
    collisionClassification: EXPERIENCE_CHANNEL_CLASSIFICATION.CANONICAL_CHANNEL_SURFACE,
    dataSource: PUBLIC_PORTAL_DATA_SOURCE.MIXED,
    dataSourceNotes: "Live club courts + hours with mock fallback (min 2). No dedicated empty UI.",
    authenticationDependency: PUBLIC_PORTAL_AUTH_DEPENDENCY.NONE,
    tenantDependency: PUBLIC_PORTAL_TENANT_DEPENDENCY.CONSUMED_SHARED,
    competitionOwnershipMarker: PUBLIC_PORTAL_COMPETITION_MARKER.NONE,
    responsiveState: EXPERIENCE_CHANNEL_READINESS.PARTIAL,
    ...
    shellPathHint: SHELL,
    evidenceReferences: ["src/pages/public/CourtsPage.jsx", SERVICE, MOCK],
  }),
  ...
  [PUBLIC_PORTAL_SURFACE_ID.PUBLIC_RANKINGS]: createPublicPortalSurfaceDescriptor({
    ...
    notes: "Only public portal page with a smoke test. Silent catch → mock in service.",
  }),
  ...
  [PUBLIC_PORTAL_SURFACE_ID.PUBLIC_NEWS]: createPublicPortalSurfaceDescriptor({
    ...
    shellPathHint: SHELL,
    evidenceReferences: ["src/pages/public/NewsPage.jsx", SERVICE, MOCK],
    notes: "Must not be certified production-ready while mock-only.",
  }),
});
...
export function getPublicPortalSharedReadinessEvidence() {
  return deepFreeze({
    seo: {
      ...
      notes:
        "App-wide index.html title/description only. No per-route document.title, Helmet, OG/Twitter, sitemap, or robots under public pages.",
      evidenceReferences: ["index.html"],
      remediableInEc01: false,
      deferReason: "Per-route SEO needs global/high-collision entrypoints or new dependencies — deferred.",
    },
    pwa: {
      state: EXPERIENCE_CHANNEL_READINESS.PARTIAL,
      notes:
