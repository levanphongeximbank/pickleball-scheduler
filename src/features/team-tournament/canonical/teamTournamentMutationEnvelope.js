...
import {
  ...
  hashUtf8Sha256Sync,
  isValidSha256Hex,
} from "./teamTournamentCanonicalDigest.js";

export const DEFAULT_ENGINE_VERSION = "team-tournament-engines@1.0.0";

/**
 * Shared canonical text for payloadHash (excludes payloadHash itself).
 * @param {Record<string, unknown>} envelope
 * @returns {string}
 */
export function serializeSetupMutationEnvelopeForPayloadHash(envelope) {
  const withoutPayloadHash = { ...envelope };
  delete withoutPayloadHash.payloadHash;
  return stableCanonicalStringify(withoutPayloadHash);
}

/**
 * Node/tests/scripts only — browser UI must use calculateSetupMutationPayloadHashAsync.
 * @param {Record<string, unknown>} envelope
 * @returns {string}
 */
export function calculateSetupMutationPayloadHash(envelope) {
  return hashUtf8Sha256Sync(serializeSetupMutationEnvelopeForPayloadHash(envelope));
}
 ...
 */
export async function calculateSetupMutationPayloadHashAsync(envelope) {
  return hashUtf8Sha256Async(serializeSetupMutationEnvelopeForPayloadHash(envelope));
}
 ...
 * @returns {object}
 */
function buildSetupMutationEnvelopeBase(params = {}) {
...
/**
 * Node/tests/scripts only — browser UI must use buildSetupMutationEnvelopeAsync.
 * @param {object} params
 * @returns {object}
 */
export function buildSetupMutationEnvelope(params = {}) {
  const envelope = buildSetupMutationEnvelopeBase(params);
 ...
 */
export async function buildSetupMutationEnvelopeAsync(params = {}) {
  const envelope = buildSetupMutationEnvelopeBase(params);
  envelope.payloadHash = await calculateSetupMutationPayloadHashAsync(envelope);
  return envelope;
}
 ...
 */
function validateSetupMutationEnvelopeShape(value) {
 ...
 * @param {string} recalculated
 * @returns {{ ok: true, envelope: object } | { ok: false, code: string, error: string }}
 */
function finishSetupMutationEnvelopeValidation(envelope, recalculated) {
  const value = canonicalizeValue(envelope);
  const shapeError = validateSetupMutationEnvelopeShape(value);
  if (shapeError) {
    return shapeError;
  }
...
/**
 * Node/tests/scripts only — browser UI must use validateSetupMutationEnvelopeAsync.
 * @param {unknown} envelope
 * @returns {{ ok: true, envelope: object } | { ok: false, code: string, error: string }}
 */
export function validateSetupMutationEnvelope(envelope) {
  try {
    ...
    return finishSetupMutationEnvelopeValidation(value, calculateSetupMutationPayloadHash(value));
  } catch (error) {
...
/**
 * @param {unknown} envelope
 * @returns {Promise<{ ok: true, envelope: object } | { ok: false, code: string, error: string }>}
 */
export async function validateSetupMutationEnvelopeAsync(envelope) {
  try {
    const value = canonicalizeValue(envelope);
    const shapeError = validateSetupMutationEnvelopeShape(value);
    if (shapeError) {
    ...
    const recalculated = await calculateSetupMutationPayloadHashAsync(value);
    return finishSetupMutationEnvelopeValidation(value, recalculated);
  } catch (error) {
    if (error instanceof CanonicalValidationError) {
      return { ok: false, code: error.code, error: error.message };
    }
