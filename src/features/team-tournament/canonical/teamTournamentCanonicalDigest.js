...
/**
 * Deterministic SHA-256 for UTF-8 text without Node crypto (browser + Node safe).
...
/**
 * Synchronous SHA-256 hex for UTF-8 text (browser + Node, no node:crypto).
 * @param {string} text
 * @returns {string}
 */
export function hashUtf8Sha256Sync(text) {
  const payload = String(text ?? "");
  return bytesToSha256Hex(sha256DigestBytes(textEncoder.encode(payload)));
}

/**
 * SHA-256 hex digest for UTF-8 text.
 * Browser: SubtleCrypto when available, else pure sync digest.
 * Node: SubtleCrypto when present, else pure sync digest.
 * @param {string} text
...
export async function hashUtf8Sha256Async(text) {
  ...
  return hashUtf8Sha256Sync(payload);
}
