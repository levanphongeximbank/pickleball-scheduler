...
test("RLS intent — user suspended → mọi permission bị chặn", () => {
  ...
  assert.equal(
    can(suspended, PERMISSIONS.VENUE_UPDATE, { venueId: "venue-a" }, RBAC_ON),
    false
  );
...
test("RLS intent — CLUB_OWNER sai club_id", () => {
  ...
  assert.equal(
    can(clubOwner, PERMISSIONS.PLAYER_UPDATE, { clubId: "club-2", venueId: "venue-a" }, RBAC_ON),
    false
  );
});
...
test("RLS intent — VENUE_MANAGER chỉ thao tác trong venue được gán", () => {
  ...
  assert.equal(can(manager, PERMISSIONS.COURT_UPDATE, { venueId: "venue-a" }, RBAC_ON), true);
  assert.equal(can(manager, PERMISSIONS.COURT_UPDATE, { venueId: "venue-x" }, RBAC_ON), false);
  assert.equal(can(manager, PERMISSIONS.SYSTEM_SETTING, {}, RBAC_ON), false);
});

test("RLS intent — CASHIER chỉ booking/payment trong venue", () => {
  const cashier = user(ROLES.CASHIER, { venueId: "venue-a" });

  assert.equal(can(cashier, PERMISSIONS.BOOKING_VIEW, { venueId: "venue-a" }, RBAC_ON), true);
  assert.equal(can(cashier, PERMISSIONS.FINANCE_EDIT, { venueId: "venue-a" }, RBAC_ON), true);
  assert.equal(can(cashier, PERMISSIONS.BOOKING_VIEW, { venueId: "venue-b" }, RBAC_ON), false);
  assert.equal(can(cashier, PERMISSIONS.PLAYER_UPDATE, { clubId: "club-1" }, RBAC_ON), false);
...
test("RLS intent — PLAYER chỉ dữ liệu club/player của mình", () => {
  ...
  assert.equal(
    can(player, PERMISSIONS.TOURNAMENT_VIEW, { clubId: "club-1", playerId: "p-1" }, RBAC_ON),
    true
  );
  assert.equal(
    can(player, PERMISSIONS.TOURNAMENT_VIEW, { clubId: "club-2", playerId: "p-1" }, RBAC_ON),
    false
  );
  assert.equal(
    can(player, PERMISSIONS.PLAYER_VIEW, { clubId: "club-1", playerId: "p-2" }, RBAC_ON),
    false
  );
  assert.equal(
    can(player, PERMISSIONS.PLAYER_VIEW, { clubId: "club-1" }, RBAC_ON),
    false
  );
});
