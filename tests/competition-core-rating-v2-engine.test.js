...
test("applyCompetitionEloFromMatchRecord is idempotent for same match id", () => {
  const clubId = "cc02-idempotent-club";
  saveClubData(clubId, {
    players: JSON.parse(JSON.stringify(players)),
    tournaments: [],
    updatedAt: new Date().toISOString(),
  ...
  const first = applyEloFromMatchRecord(clubId, record, { envSource: env });
  const second = applyEloFromMatchRecord(clubId, record, { envSource: env });
