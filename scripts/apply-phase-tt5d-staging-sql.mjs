/**
 * Phase TT-5D — Apply safety SQL on staging only.
 */
...
function assertStagingTarget(url) {
  if (String(url || "").includes(PRODUCTION_REF)) {
    console.error("❌ TT-5D: chặn apply trên Production ref.");
    process.exit(1);
  }
...
async function main() {
  ...
  console.log(dryRun ? "TT-5D dry-run complete." : "✅ TT-5D staging SQL apply complete.");
}
