/**
 * Phase TT-2D — Apply randomize/lock SQL on staging only.
 */
...
const SQL_FILE = "docs/v5/PHASE_TT2D_RANDOMIZE_LOCK_WORKFLOW.sql";

function assertStagingTarget(url) {
  if (String(url || "").includes(PRODUCTION_REF)) {
    console.error("❌ TT-2D: chặn apply trên Production ref.");
    process.exit(1);
  }
...
async function main() {
  ...
  if (!token) {
    console.log("Thiếu SUPABASE_ACCESS_TOKEN — apply thủ công qua Supabase SQL Editor staging.");
  ...
  console.log("✅ TT-2D SQL applied on staging.");
}
