/**
 * Phase TT-5B — Apply bridge/provision SQL on staging only.
 */
...
function assertStagingTarget(url) {
  if (String(url || "").includes(PRODUCTION_REF)) {
    console.error("❌ TT-5B: chặn apply trên Production ref.");
    process.exit(1);
  }
...
async function main() {
  ...
  if (dryRun) {
    console.log("TT-5B dry-run complete.");
  } else {
    console.log("✅ TT-5B staging SQL apply complete.");
  }
}
