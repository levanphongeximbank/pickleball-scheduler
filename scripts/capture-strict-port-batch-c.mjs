...
for (const screen of selectedScreens) {
  ...
  if (screen.id === "07") {
    await page.getByText("Ghép cặp / đội", { exact: true }).first().click({ force: true });
    await page.waitForTimeout(1200);
  } else if (screen.id === "08") {
    await page.getByText("Ghép cặp / đội", { exact: true }).first().click({ force: true });
    await page.waitForTimeout(1200);
    await page.getByText("Chia bảng", { exact: true }).first().click({ force: true });
    await page.waitForTimeout(1200);
  } else if (screen.id === "09") {
    await page.getByText("Vòng bảng", { exact: true }).first().click({ force: true });
    await page.waitForTimeout(1200);
  }
